...
class NotificationServiceTest extends BaseMultiSafepayTest
{
    ...
    public function setUp(): void
    {
        ...
        $mockPaymentMethod = $this->getMockBuilder(PaymentMethod::class)
            ->setConstructorArgs([
                [
                    'id' => 'IDEAL',
                    'name' => 'iDEAL',
        ...
        $mockBasePaymentOption = $this->getMockBuilder(BasePaymentOption::class)->setConstructorArgs([$allData->paymentMethod, $allData->module])->onlyMethods(['getGatewaySettings'])->getMock();

        $mockBasePaymentOption->method('getGatewaySettings')->willReturn(
            []
        );

        $mockPaymentOptionService->method('getMultiSafepayPaymentOption')->willReturn($mockBasePaymentOption);
