<div id="multisafepay-support">
    <h2 id="multisafepay-title">Documentation</h2>
    <p>{l s='Read our documentation for more information about MultiSafepay and how to get started' mod='multisafepayofficial'}</p>
    <ul class="multisafepay-ul multisafepay-lh">
        <li>
            <a href="https://docs.multisafepay.com/prestashop/" target="_blank">
                {l s='Manual' mod='multisafepayofficial'}
            </a>
        </li>
        <li>
            <a href="https://github.com/MultiSafepay/prestashop-official/blob/main/CHANGELOG.md" target="_blank">
                {l s='Changelog' mod='multisafepayofficial'}
            </a>
        </li>
        <li>
            <a href="https://docs.multisafepay.com/payments/integrations/ecommerce-platforms/prestashop-1-7/faq/" target="_blank">
                {l s='FAQ' mod='multisafepayofficial'}
            </a>
        </li>
    </ul>
    <p>{l s='For developers' mod='multisafepayofficial'}:</p>
    <ul class="multisafepay-ul multisafepay-lh">
        <li>
            <a href="https://docs.multisafepay.com/api/" target="_blank">
                {l s='API Documentation' mod='multisafepayofficial'}
            </a>
        </li>
        <li>
            <a href="https://github.com/MultiSafepay" target="_blank">
                {l s='MultiSafepay Github' mod='multisafepayofficial'}
            </a>
        </li>
    </ul>
    <h2 id="multisafepay-title">Account</h2>
    <p class="multisafepay-lh">
        {l s='To use this plugin you need a MultiSafepay account.' mod='multisafepayofficial'}.
    </p>
    <ul class="account-list">
        <li>{l s='If you would like to have a clear overview of what MultiSafepay has to offer, feel free to create a ' mod='multisafepayofficial'} <a href="https://testmerchant.multisafepay.com/signup/" target="_blank">{l s='test account' mod='multisafepayofficial'}</a>.</li>
        <li>{l s='If you would like to set up a live account, please contact the MultiSafepay sales department' mod='multisafepayofficial'}:
            <ul class="multisafepay-sales-contact-list">
                <li><strong>{l s='Netherlands' mod='multisafepayofficial'}:</strong>
                    <ul>
                        <li>
                            {l s='Telephone' mod='multisafepayofficial'}: <a href="tel:+31208500501">+31 (0)20 - 8500501</a>
                        </li>
                        <li>
                            {l s='E-mail' mod='multisafepayofficial'}: <a href="mailto:sales@multisafepay.com">sales@multisafepay.com</a>
                        </li>
                    </ul>
                </li>
                <li><strong>{l s='Belgium' mod='multisafepayofficial'}:</strong>
                    <ul>
                        <li>
                            {l s='Telephone' mod='multisafepayofficial'}: <a href="tel:+3238081241">+32 3 808 12 41</a>
                        </li>
                        <li>
                            {l s='E-mail' mod='multisafepayofficial'}: <a href="mailto:sales.belgium@multisafepay.com">sales.belgium@multisafepay.com</a>
                        </li>
                    </ul>
                </li>
                <li><strong>{l s='Spain' mod='multisafepayofficial'}:</strong>
                    <ul>
                        <li>
                            {l s='Telephone' mod='multisafepayofficial'}: <a href="tel:+34911230486">+34 911 230 486</a>
                        </li>
                        <li>
                            {l s='E-mail' mod='multisafepayofficial'}: <a href="mailto:comercial@multisafepay.es">comercial@multisafepay.es</a>
                        </li>
                    </ul>
                </li>
                <li><strong>{l s='Italy' mod='multisafepayofficial'}:</strong>
                    <ul>
                        <li>
                            {l s='Telephone' mod='multisafepayofficial'}: <a href="tel:+390294750118">+39 02 947 50 118</a>
                        </li>
                        <li>
                            {l s='E-mail' mod='multisafepayofficial'}: <a href="mailto:sales@multisafepay.it">sales@multisafepay.it</a>
                        </li>
                    </ul>
                </li>
            </ul>
        </li>
    </ul>

    <p class="multisafepay-lh" style="padding-top:20px;">
        {l s='If you would like to set up a live account, please contact the MultiSafepay sales department' mod='multisafepayofficial'}:
    </p>
    <ul class="multisafepay-ul multisafepay-lh">
        <li>
            {l s='Telephone' mod='multisafepayofficial'}:
            <a href="tel:+31208500501">
                +31 (0)20 - 8500501
            </a>
        </li>
        <li>
            {l s='E-mail' mod='multisafepayofficial'}:
            <a href="mailto:sales@multisafepay.com">
                sales@multisafepay.com
            </a>
        </li>
    </ul>
    <h2 id="multisafepay-title">Contact</h2>
    <p class="multisafepay-lh">
        {l s='Need assistance? Feel free to contact our integration team' mod='multisafepayofficial'}:
    </p>
    <ul class="multisafepay-ul multisafepay-lh">
        <li>
            {l s='Telephone' mod='multisafepayofficial'}:
            <a href="tel:+31208500500">
                +31 (0)20 - 8500500
            </a>
        </li>
        <li>
            {l s='E-mail' mod='multisafepayofficial'}:
            <a href="mailto:integration@multisafepay.com">
                integration@multisafepay.com
            </a>
        </li>
    </ul>
</div>
